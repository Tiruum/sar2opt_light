...
from src.models.llwt_v45_base import factory
from src.models.llwt_v45_base.main import LLWv4LightningModule
from src.utils.cleanup_memory import full_cleanup


os.environ['NO_ALBUMENTATIONS_UPDATE'] = '1'

CONFIG_PATH = sys.argv[1] if len(sys.argv) > 1 else 'src/models/llwt_v45_base/config.yaml'
...
def main():
    ...
    # Dataset selection: raw <-> aligned SEN12-Full via cfg.data.dataset. Both
    # modules expose an identical SEN12FullDataModule (same class + signature),
    # so only the import target and data root differ.
    ...
    if dataset_name == 'sen12_full':
        ...
        raise ValueError(
            ...
            "(expected 'sen12_full' or 'sen12_full_align')."
        )
