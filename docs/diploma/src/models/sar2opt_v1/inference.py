"""Standalone inference for SAR2OPT-V1.

Loads a Lightning checkpoint, strips the ``netG.`` prefix, runs the v1
generator on ``N_IMAGES`` SAR samples, writes side-by-side PNGs with
PSNR/SSIM.
"""
...
CHECKPOINT = "checkpoints/sar2opt_v1/sar2opt-v1.0.0-smoke-5scenes/last.ckpt"
N_IMAGES = 8
SPLIT = "val"
OUTPUT_DIR = f"./src/models/sar2opt_v1/output/{SPLIT}"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
USE_LIVE_WEIGHTS = False


def main():
    cfg = OmegaConf.load('./src/models/sar2opt_v1/config.yaml')
