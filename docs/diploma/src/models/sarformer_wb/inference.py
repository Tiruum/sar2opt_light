"""Standalone inference entry for SARFormer-WB.

Loads ``CHECKPOINT`` (Lightning ckpt), strips ``netG.`` prefix, applies the
generator to ``N_IMAGES`` SAR samples, writes side-by-side PNGs with PSNR/SSIM.
Mirrors ``src/models/huggingface_gan/inference.py``.  In sarformer-wb-3-simple
the generator returns a single tensor (the earlier ``(out, log_var, s_spk)``
tuple is gone).
"""
import os
...
CHECKPOINT = "checkpoints\\sarformer-wb\\sarformer-wb-4-sharp\\last-v1.ckpt"
N_IMAGES = 8
SPLIT = "val"      # "train" or "val"
OUTPUT_DIR = f"./src/models/sarformer_wb/output/{SPLIT}"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"


def main():
    cfg = OmegaConf.load('./src/models/sarformer_wb/config.yaml')
    ...
    netG = SARFormerWBGenerator(cfg)
    # Lightning's WeightAveraging callback overwrites ``state_dict`` with the
    # EMA-averaged weights at save time and stashes the live (non-EMA) weights
    # under ``current_model_state``.  Both dicts use the same bare key layout
    # (``netG.foo``, ``netD_main.foo``, ...) — there is no ``_average_model.``
    # prefix in the checkpoint.  So loading from ``state_dict`` automatically
    # picks up EMA weights when EMA was active during training.  Pass
    # ``USE_LIVE_WEIGHTS = True`` to load the pre-EMA copy instead.
