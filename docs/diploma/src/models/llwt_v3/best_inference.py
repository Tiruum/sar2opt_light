"""LLW-Former v0.3.x best-per-scene inference.

Iterates the full val (or train) split, scores every sample by the chosen
``METRIC`` (PSNR or SSIM), and writes the **top-K** SAR/Gen/GT side-by-side
PNGs **per scene**.  Replaces the buggy ``inference.py`` flow where the
``N_IMAGES`` knob was silently capped at one batch.

Output layout::

    OUTPUT_DIR/
      scene_5/
        top1_idx00042_psnr18.45.png
        top2_idx00310_psnr17.92.png
        top3_idx00128_psnr17.61.png
      scene_45/
        ...

Run from repo root::

    python -m src.models.llwt_v3.best_inference
"""
...
CHECKPOINT       = "checkpoints/llwt_v3/llwt-v0.3.6-lr2e4-5scenes/last.ckpt"
TOP_K            = 3
SPLIT            = "val"          # "val" or "train"
METRIC           = "ssim"         # "psnr" | "ssim" | "lpips" — score used to rank
OUTPUT_DIR       = f"./src/models/llwt_v3/output/best_{SPLIT}"
...
def main():
    cfg = OmegaConf.load('./src/models/llwt_v3/config.yaml')
    ...
    netG = LLWv3Generator(cfg).to(DEVICE).eval()
    ...
    with torch.no_grad():
        for batch_idx, (sar, opt) in enumerate(loader):
            ...
            for j in range(B):
                ...
                # FID needs uint8-equivalent normalised [0, 1] inputs with
                # ``normalize=True``.  Accumulate per-scene so each scene's
                # FID is over its own real+fake distribution.
                if COMPUTE_FID:
    ...
    for scene in sorted(per_scene.keys()):
        ...
        print(f"  scene {scene:>4s} | n={n_scene:>4d} | "
              ...
              f"mean PSNR={mean_psnr:.2f} SSIM={mean_ssim:.3f} LPIPS={mean_lpips:.3f}{fid_str}")
    print("=" * 78)
