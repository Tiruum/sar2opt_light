"""LLW-Former v0.3.0 Lightning module.

Clone of ``src/models/llwt/main.py`` with two differences:
  * Imports ``factory`` from ``src.models.llwt_v3`` (Haar-Stem ConvNeXt V2 generator
    + reused LLW discriminator).
  * Drops the ``pu_param_norm`` log line and the inline-PR helper — v0.3.0 has no
...
mode-collapse abort gates) is identical to v0.2.x.
"""
...
class LLWv3LightningModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # -------- G update --------
        # v0.3.0 has no learnable lifting -> spkdec/pr criterions are config-gated
        # to weight 0 in the v3 config, so ``need_internals`` is always False here
        # and the generator's lighter (single-output) forward path runs.
        ...
        # Optional reconstruction stack (all zero-weighted in v0.3.0; kept for
