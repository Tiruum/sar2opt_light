# src/data/sen12_full_align/datamodule.py
#
# Aligned mirror of ``src/data/sen12_full/datamodule.py``.  Identical to the raw
# datamodule except it imports the local (aligned) ``SEN12Full`` dataset and is
# meant to be pointed at ``data/sen12_full_align``.  To train on aligned pairs,
# swap this import in the model's ``train.py`` and set the data root to the
# aligned tree — no other change is required (same class API + signature).
...
from src.data.sen12_full_align.dataset import SEN12Full
