...
> **Обучение на подмножестве из 5 *репрезентативных* сцен** SEN1-2 (`5, 45, 52, 84, 100`,
> поле `data.scenes`) — сплит из Wei et al., *CFRWD-GAN* (Remote Sensing 2023), где эти
> сцены заявлены репрезентативными. Сцены покрывают разную местность, не на полном
> датасете; генерализация на полностью незнакомые регионы/сенсоры не гарантируется.
