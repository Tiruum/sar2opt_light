"""LLW-Former v0.3.0 training entry point.

Clone of ``src/models/llwt/train.py`` with:
  * ``CONFIG_PATH`` pointing at the v3 config.
  * Uses :class:`LLWv3LightningModule` (Haar-Stem ConvNeXt V2 generator).

Run from repo root::

    python -m src.models.llwt_v3.train
"""
import functools
import os
...
from src.models.llwt_v3 import factory
from src.models.llwt_v3.main import LLWv3LightningModule
from src.utils.cleanup_memory import full_cleanup


os.environ['NO_ALBUMENTATIONS_UPDATE'] = '1'

CONFIG_PATH = 'src/models/llwt_v3/config.yaml'


def _load_weights_ckpt(model: LLWv3LightningModule, ckpt_path: str,
                       use_live_weights: bool = False) -> None:
    """Load generator + discriminator weights from a Lightning checkpoint.

    ``strict=False`` so a v0.1.x or v0.2.x ckpt with a different generator
    (LLWFormerGenerator vs LLWv3Generator) won't crash; mismatched keys are
    reported but ignored.  Mainly useful for resuming a v3 run from a
    partially trained ckpt — not for cross-model warm-starting.
    """
...
def main():
    ...
    model = LLWv3LightningModule(cfg)
