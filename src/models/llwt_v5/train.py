...
Clone of ``src/models/llwt_v3/train.py`` with:
  ...
  * Uses :class:`LLWv5RefinerModule` (Haar-Stem ConvNeXt V2 + IHaar head).
...
def _load_weights_ckpt(model: LLWv5RefinerModule, ckpt_path: str,
...
def main():
    ...
    try:
        # v0.5.x SAR-DR: LLWv5RefinerModule.__init__ already loaded the frozen
        # G from cfg.system.weights_ckpt — skip the v4 helper to avoid the
        # second redundant load (which would also try to overwrite D state).
